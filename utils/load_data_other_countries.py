...
def load_data():
    ...
    gdp_data = gdp_data[gdp_data['PRICE_BASE']=='Current prices']
    y = gdp_data[['REF_AREA','TIME_PERIOD','OBS_VALUE']]
    del gdp_data
    ...
    # Rename columns
    y.rename(columns={'REF_AREA':'country', 'OBS_VALUE':'GDP'}, inplace=True)
    
    # Rename countries according to id_to_name, remove rows with countries not in id_to_name (careful you should check only with the first two letters of country)
    y['country'] = y['country'].apply(lambda x: id_to_gdp_name.get(x, None))
    y = y.dropna()
...
def load_gt_data():
    ...
    concated = pd.concat(X).reset_index(drop=True)

    X = concated.drop(['Trademark_attorney_average', 
                                        'Grants-in-Aid_for_Scientific_Research_average',
                                        'Research_&_Experimentation_Tax_Credit_average',
                                        # 'Patent_application_average',
                                        # 'Patent_infringement_average',
                                        # 'Copyright_registration_average',
                                        # 'Block_grant_average',
                                        # 'Grant_writing_average',
                                        # 'Small_Business_Innovation_Research_average'
                                        ],axis=1)
